...
public class GameResource {
			...
			try {
				JettyServer.liveGameManager.pollEvents(
						...
						(List<PollGameResponse.GameEvent> pendingEvents) -> {
							for (PollGameResponse.GameEvent e : pendingEvents) {
								try {
									System.out.println("Got game event " + Event.objectMapper.writeValueAsString(e) + " I am " + userSession.getUser().getUsername());
								} catch (JsonProcessingException ex) {
									System.out.println("Couldn't print new game event");
								}
							}
				...
				response.resume(Response.status(404).entity("Game id not found").build());
			} catch (LiveGame.UserNotInGameException e) {
				response.resume(Response.status(401).entity("User not in game").build());
			}
		});
	}
	...
	public Response makeMove(MakeMoveRequest request) {
		return AuthHelpers.withLoginAndDb(request, (UserSession userSession, Session session) -> {
			try {
				JettyServer.liveGameManager.playMove(
						request.getGameId(),
						userSession.getUser(),
						request.getMove(),
						request.getLastEventNumber()
				);
