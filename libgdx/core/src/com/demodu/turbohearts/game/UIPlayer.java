...
import static com.demodu.turbohearts.gamelogic.GameConductor.PlayerPosition.Self;
...
class UIPlayer extends DelayedPlayer {
	...
	public void getMoveImpl(ClientGameView clientGameView, MoveReporter reporter) {
		...
		for (com.demodu.turbohearts.game.GdxCard c : game.getCards()) {
			c.setSelected(false);
		}
	...
	public void reportPlayImpl(GameConductor.PlayerPosition position, Card card) {
		...
		com.demodu.turbohearts.game.GdxCard c;
		if (position == Self) {
			...
			c = new com.demodu.turbohearts.game.GdxCard(
					card.getRank(),
					card.getSuit(),
					from.x,
					from.y,
					...
					0,
					gameContext,
					com.demodu.turbohearts.game.GdxCard.State.Inactive
			);
		}
	...
	public void reportPassImpl(GameConductor.PlayerPosition position, List<Card> cards) {
		Vector2 from = game.getCoordinatesOfOpponent(position);
		ArrayList<com.demodu.turbohearts.game.GdxCard> newCards = new ArrayList<com.demodu.turbohearts.game.GdxCard>();

		for (Card c: cards) {
			com.demodu.turbohearts.game.GdxCard gc = new com.demodu.turbohearts.game.GdxCard(
					c.getRank(),
					c.getSuit(),
					from.x,
					from.y,
	...
	public void reportChargeImpl(GameConductor.PlayerPosition position, Card card) {
		...
		if (position != Self) {
			...
			game.otherChargedCards.add(new com.demodu.turbohearts.game.GdxCard(
					card.getRank(),
						card.getSuit(),
						x, y,
						game.getPlayerHand().getHeight(),
	...
	public void reportTrickEndImpl(GameConductor.PlayerPosition position) {
		...
		for (com.demodu.turbohearts.game.GdxCard c : game.onTable) {
